    ...
    <div class="col-md-10">
        <div class="table-responsive">
            <table class="table table-bordered text-center izin-table">
                ...
                <tbody>
                    @foreach($datadinas as $h)
                    <tr>
                        <td>
                            {{ \Carbon\Carbon::parse($h->tgl_awaldinas)->translatedFormat('d F Y') }} 
                                s/d 
                            {{ \Carbon\Carbon::parse($h->tgl_akhirdinas)->translatedFormat('d F Y') }}
                        </td>
                        ...
                        <td>
                            @if($h->file_surat)
                                <a href="{{ asset('uploads/surat_dinas/'.$h->file_surat) }}" target="_blank" class="btn btn-primary btn-sm">Lihat Surat</a>
                            @else
                                Tidak ada file
                            @endif
                        </td>
